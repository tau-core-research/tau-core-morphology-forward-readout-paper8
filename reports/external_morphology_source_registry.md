...
| HI_SURVEYS | gas_extent_asymmetry_memory_proxy | THINGS; LITTLE_THINGS; WALLABY; HALOGAS | hi_extent_or_asymmetry; outer-disk gas support; warp/lopsided caveat; morphology-memory/history proxy support | endpoint-selected readout family; 1D residual-derived morphology | gas morphology, extent, asymmetry, and history/memory-proxy support independent of rotation-score endpoints | heterogeneous overlap; survey-specific availability must be recorded per galaxy |
