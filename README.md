...
ready, and DustPedia, HI surveys, and PHANGS remain residual-blind acquisition
tasks. It is not an accepted morphology manifest and not an endpoint score.
...
template, completion, handoff, response-intake, promotion-gate, and missing-data
source-acquisition/source-availability stages. In the current package the overall decision is
`READY_FOR_RESIDUAL_BLIND_HUMAN_REVIEW_ONLY`: the review pipeline is actionable
for human source review, while accepted-label creation and endpoint scoring
remain disabled.
