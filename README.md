...
The deeper object behind these operational handles is the Tau
**morphological body** `M_tau`: a source-side organizing configuration whose different 4D readouts may
...
language `Theta_morph` is a morphological-body phase/history readout, while
...
projections of the same internal Tau morphological body may also affect the
...
The deeper object behind these handles is called the Tau `morphological body`
`M_tau`. This is not a visible galaxy-shape class. It is a Tau-side organizing
...
deeper Tau morphological body: baryonic distribution, metric/closure response,
...
toward the readout-relevant handle on the morphological body.
