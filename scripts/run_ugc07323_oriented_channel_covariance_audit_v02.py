#!/usr/bin/env python3
"""Re-evaluate UGC07323 side compatibility with interpolation covariance."""
...
REPORT = ROOT / "reports" / "ugc07323_oriented_channel_covariance_audit_v02.md"
C_KM_S = 299792.458
...
def main() -> None:
    first = json.loads((DATA / "ugc07323_oriented_channel_compatibility_v01.json").read_text())
    points = pd.read_csv(DATA / "ugc07323_oriented_channel_compatibility_points_v01.csv")
    ...
    result = {
        "schema": "tau_core_ugc07323_oriented_channel_covariance_audit_v02",
    ...
    (DATA / "ugc07323_oriented_channel_covariance_audit_v02.json").write_text(json.dumps(result, indent=2) + "\n")
    pd.DataFrame(covariance).to_csv(DATA / "ugc07323_oriented_channel_difference_covariance_v02.csv", index=False)
    REPORT.write_text(
        "# UGC07323 oriented-channel covariance audit v02\n\n"
