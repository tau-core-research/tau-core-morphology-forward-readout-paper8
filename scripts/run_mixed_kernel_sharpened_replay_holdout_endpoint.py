#!/usr/bin/env python3
"""Score the source-sharpened mixed replay/holdout endpoint.

Observed velocities are read only in the scoring block.  The construction reads
the frozen sharpened replay manifest unchanged.
"""

from __future__ import annotations

import itertools
import sys
from pathlib import Path

import numpy as np
import pandas as pd


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data" / "derived"
REPORTS = ROOT / "reports"
CLAIM_BOUNDARY = "mixed_kernel_sharpened_replay_holdout_endpoint_small_n_not_validation"
CASE_ORDER = ["NGC5907", "NGC7331"]

sys.path.insert(0, str(ROOT / "scripts"))
import run_source_native_readout_formula_endpoint as src  # noqa: E402
import run_s4g75_promoted_kernel_endpoint_stress_test as promoted  # noqa: E402


def rmse(obs: pd.Series, pred: pd.Series) -> float:
    return float(np.sqrt(np.mean(np.square(pred.to_numpy() - obs.to_numpy()))))


def smoothstep(x: np.ndarray) -> np.ndarray:
    clipped = np.clip(x, 0.0, 1.0)
    return clipped * clipped * (3.0 - 2.0 * clipped)


def markdown_table(df: pd.DataFrame) -> str:
    display = df.copy()
    for column in display.columns:
        if pd.api.types.is_float_dtype(display[column]):
            display[column] = display[column].map(lambda value: f"{value:.6g}")
        else:
            display[column] = display[column].astype(str)
    lines = [
        "| " + " | ".join(display.columns) + " |",
        "| " + " | ".join(["---"] * len(display.columns)) + " |",
    ]
    for _, row in display.iterrows():
        lines.append("| " + " | ".join(str(row[column]) for column in display.columns) + " |")
    return "\n".join(lines)


def build_generic_predictions() -> pd.DataFrame:
    points, _labels = src.load_points()
    points = promoted.apply_promoted_observables(points)
    points = src.add_bridge_formula_kernels(points)
    amplitudes = src.fit_amplitudes(points)
    return src.add_predictions(points, amplitudes)


def apply_formula(points: pd.DataFrame, galaxy: str, formula: pd.Series) -> pd.DataFrame:
    out = points.loc[points["galaxy"].eq(galaxy)].sort_values("r").reset_index(drop=True).copy()
    r = out["r"].to_numpy(dtype=float)
    r_inner = float(formula["r_window_inner_kpc"])
    r_outer = float(formula["r_window_outer_kpc"])
    gamma = float(formula["gamma"])
    u = (r - r_inner) / max(r_outer - r_inner, 1.0e-9)
    window = smoothstep(u)

    if str(formula["kernel_label"]) == "K_projection_source_sharpened":
        exponent = float(formula["projection_edge_exponent"])
        contrast = float(formula["truncation_contrast"])
        kernel = np.power(window, exponent) * (1.0 + contrast * window) / (1.0 + contrast)
    elif str(formula["kernel_label"]) == "K_vertical_outer_warp_source_sharpened":
        decay = float(formula["vertical_decay"])
        thickness = float(formula["projected_thickness_norm"])
        kernel = window * np.exp(-decay * np.clip(u, 0.0, 1.0)) + thickness * window * (
            1.0 - window
        )
    else:  # pragma: no cover
        raise ValueError(f"unknown kernel label: {formula['kernel_label']}")

    attenuation = np.clip(gamma * kernel, 0.0, 0.95)
    carrier_v2 = out["v_K_exponential_disk"].to_numpy(dtype=float) ** 2
    out["mixed_kernel_sharpened"] = kernel
    out["mixed_attenuation_sharpened"] = attenuation
    out["v_mixed_sharpened_replay"] = np.sqrt(np.maximum(carrier_v2 * (1.0 - attenuation), 0.0))
    out["applied_formula_source"] = str(formula["galaxy"])
    out["applied_formula_id"] = str(formula["formula_id"])
    out["applied_kernel_label"] = str(formula["kernel_label"])
    out["label_assignment_role"] = (
        "matched" if galaxy == str(formula["galaxy"]) else "wrong_label_control"
    )
    out["claim_boundary"] = CLAIM_BOUNDARY
    return out


def score_case(sub: pd.DataFrame) -> dict[str, object]:
    mixed = rmse(sub["vobs"], sub["v_mixed_sharpened_replay"])
    return {
        "galaxy": str(sub["galaxy"].iloc[0]),
        "n_points": len(sub),
        "rmse_newton": rmse(sub["vobs"], sub["vn"]),
        "rmse_tpg_v6": rmse(sub["vobs"], sub["v_v6"]),
        "rmse_mond": rmse(sub["vobs"], sub["v_mond"]),
        "rmse_exponential_disk_carrier": rmse(sub["vobs"], sub["v_K_exponential_disk"]),
        "rmse_mixed_sharpened_replay": mixed,
        "mixed_minus_newton": mixed - rmse(sub["vobs"], sub["vn"]),
        "mixed_minus_tpg_v6": mixed - rmse(sub["vobs"], sub["v_v6"]),
        "mixed_minus_mond": mixed - rmse(sub["vobs"], sub["v_mond"]),
        "mixed_minus_exponential_disk_carrier": mixed
        - rmse(sub["vobs"], sub["v_K_exponential_disk"]),
        "beats_newton": mixed < rmse(sub["vobs"], sub["vn"]),
        "beats_tpg_v6": mixed < rmse(sub["vobs"], sub["v_v6"]),
        "beats_mond": mixed < rmse(sub["vobs"], sub["v_mond"]),
        "beats_exponential_disk_carrier": mixed < rmse(sub["vobs"], sub["v_K_exponential_disk"]),
        "applied_formula_source": str(sub["applied_formula_source"].iloc[0]),
        "applied_formula_id": str(sub["applied_formula_id"].iloc[0]),
        "applied_kernel_label": str(sub["applied_kernel_label"].iloc[0]),
        "case_status": "SHARPENED_REPLAY_HOLDOUT_SCORED",
        "construction_used_vobs": False,
        "scoring_used_vobs": True,
        "claim_boundary": CLAIM_BOUNDARY,
    }


def main() -> None:
    DATA.mkdir(parents=True, exist_ok=True)
    REPORTS.mkdir(parents=True, exist_ok=True)

    freeze = pd.read_csv(DATA / "mixed_kernel_sharpened_replay_freeze_summary.csv").iloc[0]
    if freeze["freeze_status"] != "SHARPENED_REPLAY_FREEZE_READY_NOT_SCORED":
        raise RuntimeError("sharpened replay freeze is not ready")
    manifest = pd.read_csv(DATA / "mixed_kernel_sharpened_replay_freeze_manifest.csv")
    if manifest["uses_vobs_or_residual_in_construction"].any():
        raise RuntimeError("sharpened replay manifest is not endpoint-blind")

    points = build_generic_predictions()
    formula_by_source = {row["galaxy"]: row for _, row in manifest.iterrows()}

    matched_points = [
        apply_formula(points, galaxy, formula_by_source[galaxy]) for galaxy in CASE_ORDER
    ]
    matched_all = pd.concat(matched_points, ignore_index=True)
    scores = pd.DataFrame([score_case(sub) for sub in matched_points])

    matrix_rows = []
    control_points = []
    for galaxy in CASE_ORDER:
        for formula_source in CASE_ORDER:
            sub = apply_formula(points, galaxy, formula_by_source[formula_source])
            control_points.append(sub)
            matrix_rows.append(
                {
                    "galaxy": galaxy,
                    "applied_formula_source": formula_source,
                    "applied_formula_id": str(formula_by_source[formula_source]["formula_id"]),
                    "applied_kernel_label": str(formula_by_source[formula_source]["kernel_label"]),
                    "label_assignment_role": (
                        "matched" if galaxy == formula_source else "wrong_label_control"
                    ),
                    "rmse": rmse(sub["vobs"], sub["v_mixed_sharpened_replay"]),
                    "construction_used_vobs": False,
                    "scoring_used_vobs": True,
                    "claim_boundary": CLAIM_BOUNDARY,
                }
            )
    matrix = pd.DataFrame(matrix_rows)
    matrix["rank_within_galaxy"] = matrix.groupby("galaxy")["rmse"].rank(method="min").astype(int)

    by_galaxy_rows = []
    for galaxy in CASE_ORDER:
        sub = matrix.loc[matrix["galaxy"].eq(galaxy)]
        matched = sub.loc[sub["label_assignment_role"].eq("matched")].iloc[0]
        wrong = sub.loc[sub["label_assignment_role"].eq("wrong_label_control")]
        by_galaxy_rows.append(
            {
                "galaxy": galaxy,
                "matched_rmse": float(matched["rmse"]),
                "wrong_best_rmse": float(wrong["rmse"].min()),
                "matched_minus_wrong_best": float(matched["rmse"] - wrong["rmse"].min()),
                "matched_rank_within_galaxy": int(matched["rank_within_galaxy"]),
                "matched_beats_all_wrong_labels": bool((matched["rmse"] < wrong["rmse"]).all()),
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    by_galaxy = pd.DataFrame(by_galaxy_rows)

    permutation_rows = []
    for perm in itertools.permutations(CASE_ORDER):
        assignment = dict(zip(CASE_ORDER, perm))
        selected = []
        for galaxy, formula_source in assignment.items():
            selected.append(
                matrix.loc[
                    matrix["galaxy"].eq(galaxy)
                    & matrix["applied_formula_source"].eq(formula_source)
                ].iloc[0]
            )
        selected_df = pd.DataFrame(selected)
        is_matched = all(galaxy == assignment[galaxy] for galaxy in CASE_ORDER)
        permutation_rows.append(
            {
                "assignment_id": ";".join(
                    f"{galaxy}->{assignment[galaxy]}" for galaxy in CASE_ORDER
                ),
                "assignment_role": "matched_diagonal" if is_matched else "shuffled_label_null",
                "mean_rmse": float(selected_df["rmse"].mean()),
                "sum_rmse": float(selected_df["rmse"].sum()),
                "n_matched_labels": sum(galaxy == assignment[galaxy] for galaxy in CASE_ORDER),
                "claim_boundary": CLAIM_BOUNDARY,
            }
        )
    permutations = pd.DataFrame(permutation_rows).sort_values("mean_rmse").reset_index(drop=True)
    permutations["rank_by_mean_rmse"] = permutations["mean_rmse"].rank(method="min").astype(int)

    matched_perm = permutations.loc[permutations["assignment_role"].eq("matched_diagonal")].iloc[0]
    best_perm = permutations.iloc[0]
    summary = pd.DataFrame(
        [
            {
                "endpoint_status": "SHARPENED_REPLAY_HOLDOUT_2CASE_PRELIMINARY_CONTROL_RESULT",
                "n_cases_scored": len(scores),
                "mean_rmse_mixed_sharpened_replay": float(
                    scores["rmse_mixed_sharpened_replay"].mean()
                ),
                "mean_rmse_newton": float(scores["rmse_newton"].mean()),
                "mean_rmse_tpg_v6": float(scores["rmse_tpg_v6"].mean()),
                "mean_rmse_mond": float(scores["rmse_mond"].mean()),
                "mean_rmse_exponential_disk_carrier": float(
                    scores["rmse_exponential_disk_carrier"].mean()
                ),
                "n_beats_newton": int(scores["beats_newton"].sum()),
                "n_beats_tpg_v6": int(scores["beats_tpg_v6"].sum()),
                "n_beats_mond": int(scores["beats_mond"].sum()),
                "n_beats_exponential_disk_carrier": int(
                    scores["beats_exponential_disk_carrier"].sum()
                ),
                "n_matched_beats_all_wrong_labels": int(
                    by_galaxy["matched_beats_all_wrong_labels"].sum()
                ),
                "matched_permutation_rank": int(matched_perm["rank_by_mean_rmse"]),
                "best_shuffled_mean_rmse": float(best_perm["mean_rmse"]),
                "matched_minus_best_shuffled": float(
                    matched_perm["mean_rmse"] - best_perm["mean_rmse"]
                ),
                "construction_used_vobs": False,
                "scoring_used_vobs": True,
                "claim_status": (
                    "small-N sharpened replay/holdout control result; not population validation"
                ),
                "claim_boundary": CLAIM_BOUNDARY,
            }
        ]
    )

    matched_all.to_csv(
        DATA / "mixed_kernel_sharpened_replay_holdout_endpoint_points.csv", index=False
    )
    pd.concat(control_points, ignore_index=True).to_csv(
        DATA / "mixed_kernel_sharpened_replay_holdout_control_points.csv", index=False
    )
    scores.to_csv(DATA / "mixed_kernel_sharpened_replay_holdout_endpoint_scores.csv", index=False)
    matrix.to_csv(DATA / "mixed_kernel_sharpened_replay_holdout_control_matrix.csv", index=False)
    by_galaxy.to_csv(
        DATA / "mixed_kernel_sharpened_replay_holdout_control_by_galaxy.csv", index=False
    )
    permutations.to_csv(
        DATA / "mixed_kernel_sharpened_replay_holdout_shuffled_permutations.csv", index=False
    )
    summary.to_csv(DATA / "mixed_kernel_sharpened_replay_holdout_endpoint_summary.csv", index=False)

    report = [
        "# Mixed Kernel Sharpened Replay/Holdout Endpoint",
        "",
        "This endpoint scores only the frozen source-sharpened replay formulas.",
        "Observed velocities are read only by this scoring script. The result is",
        "a small-N replay/holdout control, not population validation.",
        "",
        "## Summary",
        "",
        markdown_table(summary),
        "",
        "## Scores",
        "",
        markdown_table(scores),
        "",
        "## Wrong-Label Control",
        "",
        markdown_table(by_galaxy),
        "",
        "## Shuffled Assignments",
        "",
        markdown_table(permutations),
        "",
        "## Claim Boundary",
        "",
        "This score tests whether source-sharpened kernels repair the two-case",
        "replay/holdout specificity failure. It is not a population claim and it",
        "does not update accepted single-galaxy endpoints retroactively.",
        "",
    ]
    (REPORTS / "mixed_kernel_sharpened_replay_holdout_endpoint.md").write_text(
        "\n".join(report), encoding="utf-8"
    )
    print(summary.to_string(index=False))


if __name__ == "__main__":
    main()
