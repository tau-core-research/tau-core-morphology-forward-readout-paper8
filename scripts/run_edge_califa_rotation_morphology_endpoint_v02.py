#!/usr/bin/env python3
"""Run the development-informed EDGE--CALIFA v02 endpoint once per role."""
...
from freeze_edge_califa_rotation_morphology_scoring_contract_v02 import (
    MIN_OCCUPIED_SECTORS,
    occupied_sector_jackknife_block,
)
from run_edge_califa_rotation_morphology_endpoint_v01 import (
    CALIBRATION,
    CALIBRATION_TABLE,
    C_KM_S,
    EXPECTED_MD5,
    ...
    load_terminal_frame,
    md5,
    optical_to_relativistic,
    optical_to_relativistic_jacobian,
    radial_source_support,
    radio_to_relativistic,
    radio_to_relativistic_jacobian,
    self_test,
    sha256,
)
...
def terminal_for_galaxy(
    ...
    for zone in range(N_ZONES):
        ...
        if occupied < MIN_OCCUPIED_SECTORS:
            raise RuntimeError(f"zone {zone} occupies only {occupied} terminal macrosectors")
        ...
        covariance_blocks.append(
            occupied_sector_jackknife_block(design, contrast[select], variance[select], sectors)
        )
        ...
        for mode, value in zip(("m1_cos", "m1_sin", "m2_cos", "m2_sin"), coefficient[1:5]):
            rows.append({
                ...
                "occupied_macrosectors": occupied,
                "radius_min_arcsec": float(edges[zone]),
                "radius_max_arcsec": float(edges[zone + 1]),
            })
    from freeze_edge_califa_rotation_morphology_scoring_contract_v01 import assemble_covariance
    ...
    for label in ("correct", "radial_reversal", "phase_rotation_pi_over_2", "cross_galaxy"):
        if stable_rank(sources[label]) != 16:
    ...
    differences = {label: float(scores[label]["q"] - scores["correct"]["q"]) for label in control_labels}
    return {
        "rows": rows,
        "metrics": {
            "terminal_rows_total": int(terminal_support.sum()),
            "occupied_macrosectors_by_zone": occupied_counts,
...
def write_result(role: str, result: dict[str, Any], rows: list[dict[str, Any]] | None = None) -> None:
    stem = f"edge_califa_rotation_morphology_{role}_endpoint_v02"
    ...
    (REPORTS / f"{stem}.md").write_text(
        f"# EDGE--CALIFA rotation-morphology {role} endpoint v02\n\n"
        f"Status: `{result['status']}`\n\n{result['summary']}\n\n"
        "This is development-informed external prevalidation, not a unique Tau Core "
        "prediction, a dark-sector detection, or an exhaustive standard comparator.\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    if not self_test()["all_pass"]:
        raise RuntimeError("Endpoint conversion/exact-inference self-test failed")
    ...
    if args.role == "confirmatory" and names != contract["confirmatory_galaxies"]:
        raise RuntimeError("Confirmatory membership changed")
    ...
    for index, name in enumerate(names):
        cross_name = names[(index + 1) % len(names)] if args.role == "confirmatory" else preflight["confirmatory_untouched"][0]
        try:
    ...
    if failures:
        result = {
            "schema": f"edge_califa_rotation_morphology_{args.role}_endpoint_v02",
            "status": f"{args.role.upper()}_ENDPOINT_V02_OPENED_FROZEN_GATE_FAILURE_NO_SCORE_RELEASED",
            "galaxies_opened_once": names,
            "failures": failures,
            "scores_released": False,
            "replacement_or_repair_allowed": False,
            "provenance": provenance,
            "summary": "A frozen endpoint gate failed; no partial/aggregate score is released and no same-packet repair is allowed.",
        }
        write_result(args.role, result)
        print(result["status"], failures)
        return
    rows = [row for name in names for row in opened[name]["rows"]]
    metrics = {name: opened[name]["metrics"] for name in names}
    if args.role == "development":
        result = {
            "schema": "edge_califa_rotation_morphology_development_endpoint_v02",
            "status": "DEVELOPMENT_V02_PIPELINE_EXECUTED_NO_CLAIM_NO_RETUNING",
            "galaxies_opened_once": names,
            "galaxies": metrics,
            "inference_performed": False,
            "threshold_or_protocol_change_authorized": False,
            "provenance": provenance,
            "summary": "The development galaxy passed v02. Its diagnostic values cannot alter the frozen confirmatory protocol.",
        }
        write_result(args.role, result, rows)
        print(result["status"])
        return
    ...
    status = (
        ...
        if passed else "SOURCE_DEVELOPED_EXTERNAL_MORPHOLOGY_ALIGNMENT_PREVALIDATION_FAIL"
    )
    result = {
        "schema": "edge_califa_rotation_morphology_confirmatory_endpoint_v02",
        "status": status,
        "galaxies_opened_once": names,
        "galaxies": metrics,
        ...
        "preregistered_prevalidation_pass": passed,
        "replacement_or_repair_allowed": False,
        ...
        "provenance": provenance,
        "summary": (
            f"The frozen seven-galaxy v02 endpoint returned `{status}` with exact one-sided "
            f"p={inference['primary_exact_one_sided_p']:.8f} and "
            f"{inference['positive_primary_count']}/{len(names)} positive D values."
        ),
        "claim_boundary": (
            "tests a development-informed morphology-alignment proxy beyond a declared nuisance "
            "span and equal-rank controls; does not identify a unique Tau law or exhaust standard alternatives"
        ),
    }
    write_result(args.role, result, rows)
