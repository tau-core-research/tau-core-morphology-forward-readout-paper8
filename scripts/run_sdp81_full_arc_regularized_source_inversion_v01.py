...
OUT = DATA / "sdp81_full_arc_regularized_source_inversion_v01.json"
COEFF = DATA / "sdp81_full_arc_regularized_source_coefficients_v01.npz"
LOO_CSV = DATA / "sdp81_full_arc_regularized_source_loo_v01.csv"
REPORT = ROOT / "reports/sdp81_full_arc_regularized_source_inversion_v01.md"
...
LAMBDA_GRID = np.array([1e-4, 1e-3, 1e-2, 1e-1, 1.0])
C_KM_S = 299792.458
...
def main() -> None:
    ...
    # Wrong-morphology control on the same observed pixel support: reflect the
    # fitted source map across its x axis before the frozen path operator.
    wrong_values = []
    for line_name, line in lines.items():
        for channel in range(len(VELOCITIES_KM_S)):
            source = fit_source(line["designs"], line["observations"], channel, selected_lambda, list(range(4)))
            wrong_source = source.reshape(GRID_SIDE, GRID_SIDE)[:, ::-1].ravel()
            for held in range(4):
                improvement, _, _ = held_improvement(line["designs"][held], line["observations"][held][channel], wrong_source)
                wrong_values.append(improvement)
    wrong_median = float(np.median(wrong_values))
    ...
    result = {
        "schema": "tau-core.paper8.sdp81-full-arc-regularized-source-inversion.v01",
    ...
    REPORT.write_text(
        "# SDP.81 full-arc regularized open-line source inversion v01\n\n"
        ...
        "level confirmation and cannot identify the CO(10-9) profile response.\n\n"
