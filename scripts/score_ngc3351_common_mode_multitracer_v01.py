#!/usr/bin/env python3
"""Score the frozen NGC3351 CO/Halpha common spectral mode."""
...
EXT = ROOT / "data/external/literature/ngc3351_phangs_tracer_velocity"
C = 299792.458
...
def main() -> None:
    freeze = json.loads((DATA / "ngc3351_common_mode_geometry_freeze_v01.json").read_text())
    if not freeze["freeze_complete"] or freeze["velocity_pixels_read_during_freeze"]:
        raise RuntimeError("Invalid pre-endpoint freeze")
    muse = EXT / "NGC3351_MAPS_copt_1.05asec.fits"
    co = EXT / "group.uid___A001_X2fb_X27c.lp_schinner.ngc3351_12m7mtp_co21_mom1wprior.fits"
    eco = EXT / "group.uid___A001_X2fb_X27c.lp_schinner.ngc3351_12m7mtp_co21_emom1wprior.fits"
    ...
    result = {
        "schema": "ngc3351_common_mode_multitracer_v01",
    ...
    frame.to_csv(DATA / "ngc3351_common_mode_side_spectra_v01.csv", index=False)
    summary.to_csv(DATA / "ngc3351_common_mode_multitracer_profile_v01.csv", index=False)
    (DATA / "ngc3351_common_mode_multitracer_v01.json").write_text(json.dumps(result, indent=2) + "\n")
    (ROOT / "reports/ngc3351_common_mode_multitracer_v01.md").write_text(
        "# NGC3351 common-mode multitracer preflight v01\n\n"
