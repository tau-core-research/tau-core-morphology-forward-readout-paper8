#!/usr/bin/env python3
"""Score a fixed three-mode, source-error-derived channel capacity."""
...
REPORT = ROOT / "reports" / "ngc7479_modal_capacity_scoring_v01.md"
G = 4.30091e-6
RBAR_KPC = 10.2
...
def modal_response(points: pd.DataFrame, x_eval: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    transmitted_coefficients = np.sqrt(capacities) * coefficients

    x_nodes = np.concatenate(([0.0], x))
    u_nodes = 2.0 * x_nodes / xmax - 1.0
    transmitted = basis(u_nodes) @ transmitted_coefficients
...
def main() -> None:
    freeze = json.loads((DATA / "ngc7479_modal_capacity_response_freeze_v01.json").read_text())
    ...
    for galaxy in GALAXIES:
        responses[galaxy], modes[galaxy] = modal_response(morph[morph.galaxy.eq(galaxy)], x_eval)
    summaries, details = [], []
    for floor, gas_floor in gas.groupby("grayscale_floor"):
        ...
        for i, r in enumerate(radius):
            details.append({
                ...
                "matched_modal_response": responses["NGC 7479"][i],
                "vpred_matched_km_s": predictions["NGC 7479"][i],
            })
    summary = pd.DataFrame(summaries)
    pd.DataFrame(details).to_csv(DATA / "ngc7479_modal_capacity_scoring_points_v01.csv", index=False)
    summary.to_csv(DATA / "ngc7479_modal_capacity_scoring_summary_v01.csv", index=False)
    result = {
        "schema": "tau_core_ngc7479_modal_capacity_scoring_v01",
        "status": "POST_OPEN_MODAL_CAPACITY_DIAGNOSTIC_COMPLETE",
        "freeze_status": freeze["status"], "fitted_endpoint_parameters": 0,
        "basis_order": freeze["basis_order"], "source_mode_diagnostics": modes,
    ...
    (DATA / "ngc7479_modal_capacity_scoring_v01.json").write_text(json.dumps(result, indent=2) + "\n")
    REPORT.write_text(
        "# NGC7479 modal-capacity scoring v01\n\n"
        f"Status: `{result['status']}`\n\n"
        "A fixed level/slope/curvature basis receives source-error-derived capacity per mode before finite accumulation. "
        ...
        "This is post-open and does not derive a physical channel spectrum.\n"
    )
